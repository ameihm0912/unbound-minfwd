...
RUN curl -OL https://nlnetlabs.nl/downloads/unbound/unbound-1.19.3.tar.gz && \
	tar -zxf unbound-1.19.3.tar.gz && \
	cd unbound-1.19.3 && \
...
COPY --from=builder /usr/local/lib/libunbound.so.8.1.26 /usr/local/lib/libunbound.so.8.1.26
