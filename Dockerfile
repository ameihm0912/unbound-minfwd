...
RUN curl -OL https://nlnetlabs.nl/downloads/unbound/unbound-1.17.1.tar.gz && \
	tar -zxf unbound-1.17.1.tar.gz && \
	cd unbound-1.17.1 && \
...
COPY --from=builder /usr/local/lib/libunbound.so.8.1.21 /usr/local/lib/libunbound.so.8.1.21
