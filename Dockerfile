...
RUN curl -OL https://nlnetlabs.nl/downloads/unbound/unbound-1.14.0.tar.gz && \
	tar -zxf unbound-1.14.0.tar.gz && \
	cd unbound-1.14.0 && \
...
COPY --from=builder /usr/local/lib/libunbound.so.8.1.14 /usr/local/lib/libunbound.so.8.1.14
